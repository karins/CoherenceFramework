...
def inject_errors(pe_txt, mt_txt, errors_dir, alignments, error_type, output):
    ...
    structural_errors  = inject_clausal_errors(pe, mt, get_errors(errors_dir,'structural'), markedup_corpus, output)    
    alignments_per_doc = inject_lexical_errors(pe, mt, get_errors(errors_dir,'lexical'), alignments, structural_errors, markedup_corpus, output)
    inject_discourse_errors(pe, mt, get_errors(errors_dir,'connectives'), alignments_per_doc, structural_errors, markedup_corpus, output)
                            ...
                            for idx, a in enumerate(word_alignments):
                                ...
                                if str(mt_pos)+'-' in a:
                                    ...
                                    if tagged[int(a[pe_idx+1:])][1] in  NOUNS:
                                        #markedup_corpus[docid][line_no][idx]=tag_error_attributes(error_tag, content, error_type, item)
